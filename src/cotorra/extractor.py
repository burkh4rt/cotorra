...
class Extractor:
    ...
    def extract_final(self, batch):
        ...
        first_eos = t.where(
            (hits := (input_ids == self.model.config.eos_token_id)).any(dim=-1),
            hits.to(int).argmax(dim=-1),
            input_ids.shape[-1] - 1,
        )
        with t.inference_mode():
            features = self.model(
                input_ids=input_ids, position_ids=p_ids, output_hidden_states=True
            ).hidden_states[-1]  # last hidden layer
        batch["features"] = (
    ...
    def extract(self):
        self.ds = self.loader.dataset.with_format("torch").map(
            self.extract_final,
            batched=True,
            batch_size=self.cfg.get("extract_batch_size", 8),
        )
